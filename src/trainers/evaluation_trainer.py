...
class EvaluationTrainer(object):
    ...
    def get_dataset_errors(
        ...
        issues_dup = cleaner.fraction_cut(
            scores=issues.get_issues("near_duplicates")["scores"],
            alpha=alpha,
            q=q,
            plot_result=False,
        )
        issues_ood = cleaner.fraction_cut(
            scores=issues.get_issues("irrelevants")["scores"],
            alpha=alpha,
            q=q,
            plot_result=False,
        )
        issues_lbl = cleaner.fraction_cut(
            scores=issues.get_issues("label_errors")["scores"],
            alpha=alpha,
            q=q,
            plot_result=False,
        )

        issues_dup = issues.get_issues("near_duplicates")["indices"][issues_dup]
        issues_ood = issues.get_issues("irrelevants")["indices"][issues_ood]
        issues_lbl = issues.get_issues("label_errors")["indices"][issues_lbl]
